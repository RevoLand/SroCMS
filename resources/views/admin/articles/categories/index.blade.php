    ...
    <div class="kt-subheader   kt-grid__item" id="kt_subheader">
        <div class="kt-container  kt-container--fluid ">
            <div class="kt-subheader__main">
                <h3 class="kt-subheader__title">Article Categories</h3>
                ...
                <div class="kt-subheader__breadcrumbs">
                    ...
                    <span class="kt-subheader__breadcrumbs-separator"></span>
                    <a href="{{ route('admin.articles.categories.index') }}" class="kt-subheader__breadcrumbs-link kt-subheader__breadcrumbs-link--active">Article Categories</a>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="kt-portlet__head-toolbar">
                    <div class="kt-portlet__head-actions">
                        <a href="{{ route('admin.articles.categories.create') }}" class="btn btn-primary btn-upper btn-sm btn-bold">
                            <i class="la la-edit"></i> Create New Category
                        </a>
                    </div>
                </div>
            </div>
        ...
        switch (event.target.dataset.action) {
            ...
            case 'toggle-enabled':
                axios.patch(event.target.action)
                    .then(response => {
                        $('#articlecategories-table').DataTable().draw(false);
                        swal.fire({
                            title: response.data.title,
                            html: response.data.message,
                            type: response.data.type
